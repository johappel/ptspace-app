<script lang="ts">
  ...
  import { AlertCircle, ArrowUp, BookOpen, CheckCircle2, ChevronDown, FileText, Lightbulb, MessageSquareText, Plus, ShieldCheck } from "lucide-svelte";
  import { api, type PlanningSpace, type ThinkingCard } from "$lib/api";
    ...
    messages = [
      {
        ...
        text: `Hallo, ich habe Zeit für dich. Woran möchtest du in "${space.title}" heute weiterdenken?`
      }
    ];
...
<div class="app-shell">
  <aside class="room-rail" aria-label="Planungsräume">
    ...
    <form class="new-room" on:submit|preventDefault={createSpace}>
      <label>
        Titel
        <input bind:value={newRoom.title} placeholder="z. B. Hoffnung trotz Krise" />
      </label>
      <label>
        Fach
        <input bind:value={newRoom.subject} placeholder="Religion" />
      </label>
      <label>
        Zielgruppe
        <input bind:value={newRoom.targetGroup} placeholder="Klasse 9" />
      </label>
      <label>
        Kurze Idee
        <textarea bind:value={newRoom.initialIdea} rows="3" placeholder="Woran möchtest du weiterdenken?"></textarea>
      </label>
    ...
    <div class="room-list">
      {#if loading}
        <p>Planungsräume werden geladen.</p>
      {:else if spaces.length === 0}
        <p>Noch kein Planungsraum. Lege links einen ersten Raum an.</p>
      {:else}
        {#each spaces as space}
          <button class:active={activeSpace?.id === space.id} on:click={() => openSpace(space)}>
            <span>{space.title}</span>
            <small>{space.subject || "Fach offen"} · {space.targetGroup || "Zielgruppe offen"}</small>
          </button>
        {/each}
      {/if}
    </div>
  ...
  <main class="planning-room">
    <header class="topbar">
      <div>
        <span>Gemeinsam nachdenken</span>
        <h1>{activeSpace?.title ?? "Neuer pädagogischer Denkraum"}</h1>
      </div>
      <div class="status-pill"><ShieldCheck size={16} /> Geschützter Planungsraum</div>
    </header>

    {#if error}
      <div class="notice error"><AlertCircle size={18} /> {error}</div>
    {/if}
      ...
      <section class="workspace-grid">
          ...
          <div class="messages">
            {#each messages as message}
              <article class:teacher={message.author === "teacher"} class="message">
                <div class="avatar">{message.author === "teacher" ? "L" : "CF"}</div>
                <p>{message.text}</p>
              </article>
            {/each}
            {#if sending}
              <article class="message thinking">
                ...
                <p>Ich ordne den Gedanken und aktualisiere den Denkstand.</p>
              </article>
            {/if}
          </div>
          ...
          <div class="composer-wrap">
            ...
            <form class="composer" on:submit|preventDefault={sendMessage}>
              <textarea
                ...
                rows="3"
                placeholder="Beschreibe kurz deine Unterrichtsidee oder die offene Frage."
        ...
        <aside class="thinking-panel" aria-label="Denkstand">
          <div class="panel-heading">
            <Lightbulb size={18} />
            <div>
              <strong>Denkstand</strong>
              <span>Orientierung, keine Aufgabenflut</span>
            </div>
          </div>
          ...
          {#each cards as card}
            <section class="thinking-card">
              <button class="card-toggle" on:click={() => (expandedCard = expandedCard === card.id ? "" : card.id)}>
                ...
                <span class:open={expandedCard === card.id} class="chevron"><ChevronDown size={16} /></span>
              </button>
              <p>{card.summary}</p>
              {#if expandedCard === card.id}
                <ul>
                  {#each card.previewItems.length ? card.previewItems : ["Noch nichts festgehalten."] as item}
                    <li>{item}</li>
                  {/each}
                </ul>
              {/if}
          ...
          <section class="ready-card">
            ...
            <div>
              <strong>Für den Unterricht bereit</strong>
              <span>Noch nichts freigegeben.</span>
            </div>
          </section>

          <a class="export-link" href={`http://localhost:5174/api/planning-spaces/${activeSpace.id}/export/markdown`} target="_blank" rel="noreferrer">
            <FileText size={16} /> Denkstand als Markdown ansehen
          </a>
        </aside>
      </section>
    {:else}
      <section class="empty-state">
        ...
        <h2>Lege einen Planungsraum an.</h2>
        <p>Der erste Umsetzungsschnitt arbeitet mit einer geschützten Backend-Grenze und einem simulierten Gegenüber.</p>
      </section>
    {/if}
  </main>
</div>